...
class MainMenu(linpg.SystemWithBackgroundMusic):
    def __init__(self):
        ...
        self.set_bgm_volume(linpg.setting.get("Sound", "background_music") / 100)
        # 开发者信息
        ...
        self.developer_info = linpg.converter.generate_ui(linpg.config.load(r"Data/ui.yaml", "developer_info"))
        ...
        self.exit_confirm_menu = linpg.Message(
            ...
            True, return_button=1, escape_button=1
            )
...
if GAMESTART is True and __name__ == "__main__":
    ...
    linpg.init_option_menu(
        int(linpg.display.get_width() * 0.25),
        int(linpg.display.get_height() * 0.15),
        int(linpg.display.get_width() * 0.5),
        int(linpg.display.get_height() * 0.7)
        )
    # 主菜单模块
    mainMenu = MainMenu()
    # 主循环
